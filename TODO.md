...
- [x] 실제 API 키로 동선 품질·응답 시간 확인 (2026-09-11) — 추론 끔 + 계산 메모(work) 기본: 명소 9곳 8~14초, 재시도 시 ~22초, 12건 중 11건 성공 / 추론 켬: 17~25초, 5건 중 1건 시간 초과. 자동 설정은 3~4초
- [x] 비 오는 날 실내 명소 배치·기간 부풀림 — 프롬프트 부탁 대신 서버 검증으로 강제 (맑은 날 실내 명소와 바꿀 수 있으면 거절, 기간은 기준안+1일까지)
- [x] 서버 인증 추가 — `PLANNER_TOKEN` 설정 시 앱이 `x-planner-token` 헤더로 같은 값을 보내야 통과 (미설정 시 로컬 개발용으로 인증 없음)
- [x] 검증 스크립트 `npm run check` 추가 — 알고리즘·타임라인·서버 검증·자동 설정 보정 30여 개 케이스, 외부 API 호출 없음
- [ ] 실제 API 호출로 새 검증 규칙(날씨 배치·기간 제한) 재검증 — 기기 테스트와 함께 진행
- [x] "자동 설정" 플로우에 자유 텍스트 입력 → Structured Outputs로 TripSettings 구조화 변환 기능 연동 (AutoSetupScreen, `POST /api/trip-settings`)
...
- [ ] 푸시 알림 (여행 전날 일정 알림)
- [x] "자동 설정" 모드 실제 동작 구현 — 홈의 자동 설정 버튼이 AutoSetupScreen으로 연결됨
- [x] 날씨 선택 제거 후 남은 `weather: 'sunny'` 고정값 정리 — TripSettings에서 weather 필드 제거, 타임라인의 "맑음" 태그 삭제 (실제 날씨는 WeatherScreen 예보로만 사용)
- [x] 저장된 일정을 다시 저장할 때 이름 입력칸이 "제주 여행"으로 초기화되는 문제 — 기존 일정 이름으로 시작하도록 수정

...
import { ACCOMMODATION_COORDS } from './generateTimeline';


type MealKind = 'lunch' | 'dinner';
type PlanInput = Pick<TripSchedule, 'spots' | 'settings' | 'accommodation' | 'customAccommodationCoords'>;
...
// 하루를 넘긴 시각(24:20 같은 값)을 그대로 내보내지 않는다 — generateTimeline과 같은 규칙
...
function layoutDays(
  ...
  const accomCoords = accommodation === 'custom' && schedule.customAccommodationCoords
  ...
  for (let d = 0; d < totalDays; d++) {
    ...
    // 식당 후보는 타임라인이 확정된 뒤 api/mealRecommend가 한꺼번에 채운다 (여기선 빈 options)
    ...
    while (queue.length > 0) {
      ...
      if (asMeal) {
        ...
        if (mealOnArrival) pushMeal(mealOnArrival);
        items.push({
          type: 'spot',
          time: formatTime(cursor),
          name: spot.name,
  ...
  return { dayPlans, leftover: queue.length };
}

...
## [2026-09-12] AI 동선 실측 검수 — 추론(thinking) 트레이드오프와 서버 검증 규칙 추가

### 배경
실제 Claude 키로 처음 호출해 응답 시간과 동선 품질을 측정. 추론 설정에 따라 결과가 크게 갈렸고, 타임라인 배치에서 새 버그도 드러남.

### 측정 (명소 9곳, `claude-sonnet-5`, effort low)
| 설정 | 응답 시간 | 결과 |
|------|-----------|------|
| 추론 켬(adaptive) | 17~25초 | 5건 중 4건 성공, 1건 시간 초과(45초) |
| 추론 끔 | 6~9초 | 4건 중 2건 성공 — 하루 한도 계산을 자주 틀림 |
| **추론 끔 + 계산 메모** | **8~14초** (재시도 시 ~22초) | **12건 중 11건 성공 → 기본값 채택** |

계산 메모는 명소 목록(`spots`)을 확정하기 전에 `work` 필드에 "그날 날씨 | 이동·체류 합계 = N / 한도 M"을 먼저 쓰게 하는 방식. 긴 추론 대신 짧은 계산만 적게 해서 속도와 정확도를 함께 잡음. 이 필드는 앱으로 전달하지 않음. `PLANNER_THINKING=adaptive`로 추론을 다시 켤 수 있음.

### 드러난 문제와 대응
1. **점심이 빠짐** — 긴 이동이 점심 시간을 통째로 가로지르는 경우(동문시장 09:10 출발 → 성산일출봉 12:47 도착). 도착 시각이 식사 시간대면 도착 후 먼저 먹도록 `generateTimeline`과 `feasibility.simulateDayEnd`를 같은 규칙으로 수정.
2. **불가능한 날짜 요약** — 09시 출발인데 "성산일출봉 일출 감상", 14시 방문인데 "천지연폭포 야경". 계절별 일출·일몰 시각으로 "이 활동 시간에는 일출·야경을 볼 수 없다"를 요청마다 구체적으로 전달.
3. **비 오는 날에 야외 명소** — 프롬프트로 부탁하는 방식은 추론을 끄면 지켜지지 않음. 서버가 명소별 실내·야외를 분류해 프롬프트에 표시하고, 맑은 날 실내 명소와 바꿀 수 있으면 검증에서 거절.
4. **기간 부풀림** — 하루에 한 곳만 넣어 9곳을 7일로 계획. 기준안(단순 알고리즘)+1일을 넘으면 거절.
5. **구조화 출력의 `enum`이 강제되지 않음** — SDK가 스키마를 만들 때 허용 값 목록을 제약이 아닌 설명문으로 옮김. 자동 설정 값은 문자열로 받고 서버에서 기본값으로 보정(잘못된 값 하나로 기능 전체가 실패하지 않도록).

### 검증
- `npm run check` 신설 — 알고리즘·타임라인·서버 검증·자동 설정 보정 30여 개 케이스. 외부 API를 호출하지 않아 키 없이 실행 가능. 전부 통과.
- 인증 토큰: 토큰 없음·틀림은 401, 올바른 토큰은 통과(입력 검증 400까지 도달) 확인.

### 수정 파일 요약
| 파일 | 변경 |
|------|------|
| `server/src/claude.ts` | Claude 클라이언트 공용 모듈, `PLANNER_THINKING`·`ANTHROPIC_WORKSPACE_ID` 지원 |
| `server/src/schema.ts` | 계산 메모(`work`) 필드 추가 |
| `server/src/prompt.ts` | 실내·야외 표시, 일출·야경 제한, 계산 메모 작성법 |
| `server/src/feasibility.ts` | 실제 시각 시뮬레이션, 기간 과다·날씨 배치 검증 |
| `server/src/index.ts` | `PLANNER_TOKEN` 인증 |
| `src/algorithms/generateTimeline.ts` | 이동 중 식사 시간 도달 시 도착 후 식사 |
| `scripts/check.ts` | 회귀 테스트 (`npm run check`) |
